### RScript for visualising mapping statistics

library(tools)

### Import mapping statistics
tbl <- read.table("./StarBAM/Log/outputMappingStats.txt", header=T, row.names=1)
rownames(tbl) <- basename(file_path_sans_ext(file_path_sans_ext(rownames(tbl))))  # Extract file basename

### Extract read lengths, and plot
